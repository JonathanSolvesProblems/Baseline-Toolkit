 ...
 */
export function loadConfig(configPath?: string, startDir?: string): BaselineConfig {
  const searchPaths = [];
  ...
  if (configPath) {
    ...
    while (currentDir !== dirname(currentDir)) {
      searchPaths.push(
        join(currentDir, 'baseline.config.json'),
        join(currentDir, '.baseline.json'),
 ...
 */
export function validateConfig(config: Partial<BaselineConfig>): BaselineConfig {
  const validated: BaselineConfig = {
    rules: {
